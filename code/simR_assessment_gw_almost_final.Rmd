...
# adapted pwr.r.test() function to deal with errors caused by uniroot() where the endpoints of an interval were not of opposite signs
...
simulation_summary <- simulation |>
  ...
  summarize(sig_rate_orig = round_half_up(mean(p_orig < .05), digits = 3), # create new column with proportion of sig. results in original studies
            ...
            proportion_agree = round_half_up(mean(p_agree), digits = 3), # proportion of agreement between original and replication studies
...
simulation_summary <- simulation |>
  ...
  summarize(proportion_diff = round_half_up(mean(diff), digits = 3), #calculate proportion of significant differences
...
simulation_summary <- simulation |>
  ...
  summarize(proportion_diff_q15 = round_half_up(mean(diff_q15), digits = 3), #calculate proportion of true differences Cohen's q |.15|
            mean_cohen_q = round_half_up(mean(cohen_q), digits = 3), # mean of Cohen's q, effect size measure for differences
            mean_estimate_orig = round_half_up(mean(r_orig), digits = 3), # (biased) mean estimate original studies 
            mean_estimate_rep = round_half_up(mean(r_rep), digits= 3)) |>  # mean estimate replication studies
